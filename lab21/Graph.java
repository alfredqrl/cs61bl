...
public class Graph implements Comparator<Integer> {
    ...
    public Graph prims(int start) {
        ...
        TreeMap<Integer, Edge> distTo = new TreeMap<>();
        PriorityQueue<Integer> PQ = new PriorityQueue<>(Comparator.comparingInt(v -> distTo.get(v).getWeight()));
        TreeSet<Integer> all = getAllVertices();

        PQ.add(start);
        distTo.put(start, new Edge(0, 0 ,0));
        for(int v : all) {
            result.addVertex(v);
        }
        while (!PQ.isEmpty()) {
            int p = PQ.poll();
            if (all.contains(p)){
                for (Edge q : getEdges(p)) {
                    int des = q.getDest();
                    if (all.contains(des)){
                        if (! distTo.containsKey(des)){
                            distTo.put(des, q);
                            PQ.add(des);
                        }
                        else if(q.getWeight() < distTo.get(des).getWeight()){
                            distTo.put(des, q);
                            PQ.add(des);
                        }
                    }
    ...
    public void update(List<Integer> in) {
        distFromTree.clear();

...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
    ...
    private void printHelper(String help){
        help += "    ";
        Amoeba curr = this.root;
        if (curr.getChildren() == null) {
            return;
        } else {
            for (Amoeba a : curr.children) {
    ...
    public class AmoebaDFSIterator implements Iterator<Amoeba> {
        ...
        public Amoeba next() {
            ...
            Amoeba curr = fringe.pop();
            if (curr.children != null){
                int childSize = curr.children.size();
                while (childSize > 0){
                    fringe.push(curr.children.get(childSize - 1));
                    childSize--;
                }
            }
            return curr;

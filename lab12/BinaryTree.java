public class BinaryTree<T> {
    ...
    private int heightHelper(TreeNode help){
        if (help == null){
            return 0;
        }else {
            int lHeight = heightHelper(help.left);
            int rHeight = heightHelper(help.right);
            if (lHeight > rHeight) {
                return (lHeight + 1);
            } else {
                return (rHeight + 1);
    ...
    public TreeNode fibTreeHelper(int n){
        ...
        Integer a = (Integer) left.item + (Integer) right.item;
        return new TreeNode(a,left, right);

...
public class Main {
    ...
    public static void writeStory(String[] args) {
        ...
        // FIXME
        String outFile;
        File f1 = new File(String.valueOf(Utils.join(".capers", "story")));
        if (f1.exists()){
            outFile = Utils.readContentsAsString(f1);
            outFile = outFile + "\n" + args[1];
        }else{
            outFile = args[1];
        }
        Utils.writeContents(f1,outFile);
        //Utils.writeContents(f1, args[1]);
        String outputFile = Utils.readContentsAsString(f1);
    ...
    public static void celebrateBirthday(String[] args) {
        ...
        Dog returnDog;
        try {
            returnDog = Dog.fromFile(args[1]);
            returnDog.haveBirthday();
            returnDog.saveDog();
        }catch (Exception e){
            exitWithError("Invalid");
        }
        //System.out.println(returnDog.toString());
    }
